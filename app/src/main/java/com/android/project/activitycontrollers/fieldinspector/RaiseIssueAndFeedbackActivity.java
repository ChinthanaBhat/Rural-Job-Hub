...
public class RaiseIssueAndFeedbackActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_raise_issue_and_feedback);
        ...
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(RaiseIssueAndFeedbackActivity.this, error.toString(), Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    public void submit(View view){
        descriptionET=findViewById(R.id.issue_feedback);

        String description=descriptionET.getText().toString();

        IssueAndFeedback raiseIssueAndFeedback=new IssueAndFeedback();
        raiseIssueAndFeedback.setDescription(description);
        raiseIssueAndFeedback.setJobId(jobList.get(jobSP.getSelectedItemPosition()).getId());
        Calendar date = Calendar.getInstance();
        SimpleDateFormat dateFormatter = new SimpleDateFormat("yyyy-MM-dd", Locale.US);
        String stringDate = dateFormatter.format(date.getTime());
        raiseIssueAndFeedback.setDate(stringDate);

        DatabaseReference databaseReference= FirebaseDatabase.getInstance().getReference();
        String key=databaseReference.child("Issue_Feedback").push().getKey();
        databaseReference.child("Issue_Feedback").child(key).setValue(raiseIssueAndFeedback);
        raiseIssueAndFeedback.setId(key);
        if (raiseIssueAndFeedback.getId() != null) {
            ...
            Intent intent = new Intent(this, IssueAndFeedback.class);
            startActivity(intent);
        }
    }
